...
for epoch in range(num_train_epochs):
    ...
    if epoch < num_warm_epochs:
        ...
        _,train_results = tnt.train(model=ppnet_multi, dataloader=train_loader, optimizer=warm_optimizer, args=args,
                      class_specific=class_specific, coefs=coefs, log=log)
    else:
        tnt.joint(model=ppnet_multi, log=log)
        joint_lr_scheduler.step()
        _,train_results = tnt.train(model=ppnet_multi, dataloader=train_loader, optimizer=joint_optimizer, args=args,
                      class_specific=class_specific, coefs=coefs, log=log)
